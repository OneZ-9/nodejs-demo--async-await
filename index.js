...
const getDogPic = async () => {
  try {
    ...
    const Promise1res = superagent.get(
      `https://dog.ceo/api/breed/${data}/images/random`
    );
    const Promise2res = superagent.get(
      `https://dog.ceo/api/breed/${data}/images/random`
    );
    const Promise3res = superagent.get(
      `https://dog.ceo/api/breed/${data}/images/random`
    );

    const all = await Promise.all([Promise1res, Promise2res, Promise3res]);
